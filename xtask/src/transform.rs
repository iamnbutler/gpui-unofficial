...
fn add_proptest_dependency(doc: &mut DocumentMut) {
    ...
    // Add proptest to test-support feature. We use the bare name (not `dep:proptest`)
    // so cargo creates the implicit `proptest` feature — gpui's source has
    // `#[cfg(feature = "proptest")]` references that require it to exist.
    if let Some(features) = doc.get_mut("features") {
        if let Some(table) = features.as_table_like_mut() {
