...
fn add_proptest_dependency(doc: &mut DocumentMut) {
    ...
    // Enable proptest in the test-support feature using the BARE feature name,
    // not `dep:proptest`. The optional `proptest` dependency above creates an
    // implicit `proptest` feature, and zed's source gates code on
    // `#[cfg(feature = "proptest")]` (e.g. gpui's `color.rs`). Referencing the
    // dependency as `dep:proptest` anywhere in [features] tells Cargo to stop
    // creating that implicit feature, which turns `feature = "proptest"` into an
    // "unexpected cfg" warning — a hard error under `RUSTFLAGS=-Dwarnings` (which
    // the sync workflow sets). Using the bare name keeps the implicit feature and
    // matches zed's own upstream configuration.
    if let Some(features) = doc.get_mut("features") {
        if let Some(table) = features.as_table_like_mut() {
