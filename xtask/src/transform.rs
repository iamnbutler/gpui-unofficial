...
"#,
    ...
    /// Under `use_local_deps`, an internal sibling dependency must carry both
    /// `path` (so `cargo check` resolves it directly inside the local
    /// transform output) and `version` (required by `cargo package`, which
    /// rejects path dependencies with no version). `[patch.crates-io]` is
    /// deliberately NOT used here: `cargo package`'s lockfile-resolution phase
    /// ignores `[patch]`/`[replace]` sections entirely, so a patch table can
    /// never make that resolution succeed pre-publish. `test-isolated`
    /// instead satisfies this version requirement with a vendored-sources
    /// directory built at test time (see `test_isolated.rs`).
    #[test]
    fn local_deps_use_path_and_version_for_internal_siblings() {
        let dir = tempfile::tempdir().unwrap();
...
"#,
        ...
        assert_eq!(
            doc["dependencies"]["gpui_util"]["path"].as_str(),
            Some("../gpui-util-gpui-unofficial"),
            "internal sibling dep must carry a path, got:\n{out}"
        );
        assert_eq!(
            doc["dependencies"]["gpui_util"]["version"].as_str(),
            Some("1.21.0"),
            "internal sibling dep must also carry a version, got:\n{out}"
        );
        assert!(
            doc.get("patch").is_none(),
            "local-deps transform must not add a patch section, got:\n{out}"
        );

        // The version-deps pass (real publish, not local verification) must
        // never emit a path either — only a version, resolved from crates.io.
        fs::write(
            crate_dir.join("Cargo.toml"),
            r#"[package]
name = "collections"
