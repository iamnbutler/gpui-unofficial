...
fn add_proptest_dependency(doc: &mut DocumentMut) {
    ...
    // Use the implicit feature name (not `dep:proptest`) so that
    // `#[cfg(feature = "proptest")]` in source code resolves — using any
    // `dep:` syntax suppresses Cargo's implicit feature for the optional dep.
    if let Some(features) = doc.get_mut("features") {
        if let Some(table) = features.as_table_like_mut() {
