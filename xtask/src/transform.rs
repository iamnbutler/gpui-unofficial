...
/// Map from original module name to unofficial module name (for use statements)
fn unofficial_module_name(name: &str) -> String {
    // Keep snake_case, add suffix
    format!("{name}_unofficial")
}

pub fn run(zed_tag: &str, zed_path: Option<&str>, output_dir: &str) -> Result<()> {
    ...
    for crate_name in CRATE_PUBLISH_ORDER {
        ...
        transform_crate(&zed_dir, &output_path, crate_name, &workspace_deps, zed_tag)?;
    }
...
fn checkout_tag(path: &Path, tag: &str) -> Result<()> {
    ...
    if !status.success() {
        bail!("Failed to checkout tag {tag}");
    }
...
fn transform_crate(
    ...
    let src_dir = zed_dir.join("crates").join(crate_name);
    if !src_dir.exists() {
    ...
    transform_cargo_toml(&dest_dir, crate_name, workspace_deps, zed_tag)?;
...
fn transform_cargo_toml(
    ...
    transform_dependencies(&mut doc, "dependencies", workspace_deps, &version)?;
    transform_dependencies(&mut doc, "dev-dependencies", workspace_deps, &version)?;
    transform_dependencies(&mut doc, "build-dependencies", workspace_deps, &version)?;
                        ...
                        for dep_section in ["dependencies", "dev-dependencies", "build-dependencies"] {
                            if table.contains_key(dep_section) {
                                ...
                                if let Some(deps) = table.get(dep_section).cloned() {
                                    ...
                                    transform_dependencies(&mut temp_doc, dep_section, workspace_deps, &version)?;
    ...
    // Remove inspector feature from gpui_macros
    if original_name == "gpui_macros" {
...
fn transform_dependencies(
    ...
    for dep_name in dep_names {
        let is_internal = CRATE_PUBLISH_ORDER.contains(&dep_name.as_str());
